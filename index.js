...
app.post("/createuser", (req, res) => {
  ...
  misdb.query(query1, [data.user_email_id, data.first_name, data.middle_name, data.last_name,data.password, data.designation, data.phone_no, data.profile_picture, data.superior_name, data.superior_email, currentDateTime, data.emp_id, data.last_pass_change, data.login_disabled_date, data.fpi_template, data.fpi_template_two, data.fpi_template_three, data.fpi_template_four, data.lang, data.locations, data.user_type], (err, results) => {
    ...
    misdb.query(query2, [data.user_id,data.sl_id], (err, results) => {
      if (err) {
      ...
      misdb.query(query3, [data.user_id, data.user_name, data.action_name, data.start_date, data.system_ip, data.remarks], (err, results) => {
        if (err) {
       ...
       const query4 = "INSERT INTO tbl_bridge_role_to_um (role_id, user_ids) VALUES (?, ?)";
        misdb.query(query4, [data.role_id,user_id, data.user_ids], (err, results) => {
          if (err) {
          ...
          misdb.query(query5, [data.group_id,user_id, data.user_ids], (err, results) => {
            if (err) {
...
app.post("/login", (req, res) => {
  ...
  misdb.query(selectQuery, [user_email_id], (err, rows) => {
    ...
    bcrypt.compare(password, hashedPassword, (err, result) => {
      ...
      if (result) {
        // Update last_active_login timestamp in the database
        ...
        misdb.query(updateQuery, [user_email_id], (err) => {
          ...
          return res.status(200).json({ message: "Login successful", user_id: userData.user_id });
        });
      } else {
